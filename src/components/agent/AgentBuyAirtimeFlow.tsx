...
interface AgentBuyAirtimeFlowProps {
  walletBalance: number;
  onOrderCreated: (order: Order) => void;
  onOpenReceipt: (order: Order) => void;
}

export const AgentBuyAirtimeFlow: React.FC<AgentBuyAirtimeFlowProps> = ({
  walletBalance,
  onOrderCreated,
  onOpenReceipt,
}) => {
  const [step, setStep] = useState<
    "network" | "amount" | "recipient" | "review" | "processing" | "success"
  >("network");
    ...
    const newOrder: Order = {
      ...
      deliveryTimeline: [
        ...
        },
        {
          ...
          timestamp: "10:00:08",
          status: "completed",
          note: `GH₵${finalAmount.toFixed(2)} recharge complete`,
        },
      ],
    };
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      {step !== "processing" && step !== "success" && (
        <div className="flex gap-3">
          ...
          {step !== "review" ? (
            <Button onClick={handleNext} className="flex-1 h-10">
              Next
              <ArrowRight className="w-4 h-4 ml-2" />
            </Button>
          ) : (
            <Button onClick={handleSubmit} className="flex-1 h-10">
              Complete Purchase
