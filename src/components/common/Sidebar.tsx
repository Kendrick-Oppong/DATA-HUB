...
export const Sidebar: React.FC<SidebarProps> = ({
    ...
    return (
      <div className="border-t border-border/70 p-2.5 bg-muted/20">
        <button
          type="button"
          onClick={() => select('profile')}
          className={`group flex w-full items-center gap-3 rounded-2xl border p-2 text-left transition-all duration-150 cursor-pointer ${
            isProfileActive
  ...
  return (
    <TooltipProvider>
      ...
      {/* ============================================================ */}
      <aside
        className={`hidden md:flex fixed inset-y-16 left-0 z-30 flex-col border-r border-border bg-card/95 backdrop-blur-xs transition-[width] duration-300 ease-in-out ${
          isCollapsed ? 'w-20' : 'w-64'
        }`}
      >
        {/* Top Role Header */}
        <div
          className={`flex h-14 items-center border-b border-border/70 px-4 ${
        ...
        <nav className="min-h-0 flex-1 overflow-y-auto p-3 space-y-4">
        ...
        {isMobileOpen && (
          <div className="fixed inset-0 z-50 md:hidden flex">
            ...
            >
              ...
              {/* Sheet Scrollable Navigation */}
              <nav className="min-h-0 flex-1 overflow-y-auto p-3.5 space-y-4">
