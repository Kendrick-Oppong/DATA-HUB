...
export const PublicStorefront: React.FC<PublicStorefrontProps> = ({
  ...
  const [selectedNetwork, setSelectedNetwork] = useState<TelecomNetwork>(
    storeConfig.defaultNetwork || "MTN",
  );
  const [selectedBundleId, setSelectedBundleId] = useState<string>(() => {
    const match = bundles.find(
      (b) => b.network === (storeConfig.defaultNetwork || "MTN"),
    );
  ...
  const agentMargin = currentBundle
    ? Math.max(
        0,
  ...
  const handleStartCheckout = (e: React.FormEvent) => {
    ...
    const newOrder: Order = {
      ...
      deliveryTimeline: [
        {
          step: "Order Placed on Reseller Store",
          timestamp: "10:00:01",
          status: "completed",
        },
        {
          step: "MoMo Payment Received",
          timestamp: "10:00:04",
          status: "completed",
        },
        {
          step: "SDH EVD Gateway Dispatched",
          timestamp: "10:00:09",
          status: "completed",
        },
        {
          step: "Data Balance Credited",
          timestamp: "10:00:15",
          status: "completed",
  ...
  const openWhatsAppHelp = () => {
    ...
    window.open(
      `https://wa.me/233${storeConfig.whatsappNumber.replace(/^0/, "")}?text=${text}`,
      "_blank",
    );
  };
    ...
    <div className="max-w-4xl mx-auto space-y-6 pb-16">
          ...
          <div className="p-6 rounded-3xl bg-card border border-border/80 shadow-xs space-y-4">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
              <div>
                ...
                <h2 className="text-base font-extrabold text-foreground">
                  Choose{" "}
                  {selectedNetwork === "AirtelTigo" ? "AT" : selectedNetwork}{" "}
                  Package
                </h2>
              </div>
          ...
          <div className="p-6 rounded-3xl bg-card border border-border/80 shadow-xs space-y-5">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-0.5">
                Data is delivered directly to this Ghana number via carrier EVD
                switch.
              </p>
            </div>
      ...
      {checkoutStep === "completed" && completedOrder && (
        <div className="p-8 sm:p-12 rounded-3xl bg-card border border-emerald-500/30 shadow-2xl text-center space-y-6 animate-in fade-in-50 zoom-in-95">
          ...
          <div className="space-y-2 max-w-md mx-auto">
            ...
            <p className="text-xs sm:text-sm text-muted-foreground">
              <strong>{currentBundle.sizeLabel} Non-Expiry Data</strong> has
              been dispatched to{" "}
          ...
          <div className="max-w-md mx-auto p-4 rounded-2xl bg-muted/40 border border-border/80 text-left space-y-2 text-xs">
            ...
            <div className="flex justify-between items-center">
              ...
              <span className="text-foreground">{completedOrder.date}</span>
            </div>
          </div>
      ...
      <div className="p-5 rounded-3xl bg-card border border-border/80 flex flex-col sm:flex-row justify-between items-center gap-3 text-xs text-muted-foreground shadow-xs">
        <div className="flex items-center gap-2">
          ...
          <span>
            Powered by <strong>Smart Data Hub Ghana</strong> Telecom
            Infrastructure
          </span>
        </div>
        <button
