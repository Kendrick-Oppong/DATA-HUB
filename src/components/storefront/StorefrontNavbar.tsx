      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 sm:h-20 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3 sm:gap-4 min-w-0">
          {store.storeLogo ? (
            ...
            >
              {store.storeName ? store.storeName.slice(0, 2).toUpperCase() : "ST"}
            </div>
          )}
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-2">
              ...
              <span className="hidden sm:inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-emerald-500/15 text-emerald-700 dark:text-emerald-400 text-[10px] font-black border border-emerald-500/30 shrink-0">
        ...
        <div className="hidden lg:flex items-center gap-2 px-3 py-1.5 rounded-2xl bg-muted/50 border border-border/80 text-xs">
          ...
          <span className="font-bold text-foreground">
            {isLive ? "Ghana Telecom Gateways 100% Online" : "Scheduled Maintenance Mode"}
          </span>
        </div>
        ...
        <div className="flex items-center gap-2 sm:gap-2.5">
          ...
          {/* Back to Smart Data Hub portal */}
          <Button
            type="button"
            variant="ghost"
            size="sm"
            onClick={onSwitchToSDH}
            className="h-9 px-2.5 rounded-xl text-muted-foreground hover:text-foreground font-bold text-xs flex items-center gap-1 cursor-pointer"
            title="Smart Data Hub Platform"
          >
            <Layers className="w-3.5 h-3.5" />
            <span className="hidden xl:inline">SDH Hub</span>
          </Button>
      ...
      {mobileMenuOpen && (
        <div className="sm:hidden border-t border-border bg-card/95 backdrop-blur-md p-4 space-y-3 animate-in slide-in-from-top-2">
          <div className="flex items-center justify-between pb-2 border-b border-border/80">
            <span className="text-xs font-bold text-muted-foreground">Store Navigation</span>
            <span
